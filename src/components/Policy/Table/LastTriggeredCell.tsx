...
export const LastTriggeredCell: React.FunctionComponent<LastTriggeredCellProps> = (props) => {
    ...
    if (props.lastEvaluation) {
        ...
        if (isAfter(now, oneMonthAfterLastTriggered)) {
            lastEvaluationString = `${format(props.lastEvaluation, 'MMM dd y')}  ${Messages.components.lastTriggeredCell.ago}`;
        } else {
            lastEvaluationString = formatDistanceToNow(props.lastEvaluation);
        }
    } else {
